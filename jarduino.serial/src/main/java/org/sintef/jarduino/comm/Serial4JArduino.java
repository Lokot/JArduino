...
public class Serial4JArduino implements JArduinoSerial<SerialConfiguration> {
	...
	static {
		...
		try {
			...
			if (osName.equals("Mac OS X")) {
				NativeLibUtil.copyFile(Serial4JArduino.class.getClassLoader()
			...
			if (osName.equals("Linux") && osProc.equals("x86-64")) {
				NativeLibUtil.copyFile(Serial4JArduino.class.getClassLoader().getResourceAsStream(
						"nativelib/Linux/x86_64-unknown-linux-gnu/librxtxSerial.so"), "librxtxSerial.so");
			}
			...
			if (osName.equals("Linux") && osProc.equals("x86")) {
				NativeLibUtil.copyFile(Serial4JArduino.class.getClassLoader().getResourceAsStream(
						"nativelib/Linux/i686-unknown-linux-gnu/librxtxParallel.so"), "librxtxParallel.so");
				NativeLibUtil.copyFile(Serial4JArduino.class.getClassLoader().getResourceAsStream(
	...
	public Serial4JArduino(String port) throws JArduinoConnectionException {
		this.port = port;
		init();
	}

	public Serial4JArduino(String port, SerialConfiguration conf) throws JArduinoConnectionException {
		this.port = port;
		this.conf = conf;
		init();
	}

	void connect(String portName, SerialConfiguration conf) throws JArduinoConnectionException {
		...
		try {
			...
			if (portIdentifier.isCurrentlyOwned()) {
				...
				throw new JArduinoConnectionException("Error: Port " + portName + " is currently in use");
			} else {
				...
				if (commPort instanceof SerialPort) {
					...
					throw new JArduinoConnectionException("Error: Port " + portName + " is not a valid serial port.");
				}
			}
		} catch (NoSuchPortException e) {
			throw new JArduinoConnectionException("Error: Port " + portName + " is not exist", e);
		} catch (PortInUseException e) {
			throw new JArduinoConnectionException("Error: Port " + portName + " is currently in use", e);
		} catch (UnsupportedCommOperationException e) {
			throw new JArduinoConnectionException("Error: Port " + portName, e);
		} catch (IOException e) {
			throw new JArduinoConnectionException("Error: Port " + portName, e);
	...
	/**
	 * @return A HashSet containing the CommPortIdentifier for all serial ports that
	...
	public static String selectSerialPort() {
		...
		return (String) JOptionPane.showInputDialog(null, "JArduino", "Select serial port", JOptionPane.PLAIN_MESSAGE,
